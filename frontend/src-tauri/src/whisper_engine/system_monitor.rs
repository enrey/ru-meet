...
pub struct SystemMonitor {
    system: Arc<RwLock<Option<System>>>,
...
impl SystemMonitor {
    pub fn new() -> Self {
        // Creating `System` can block on some Windows hosts. The monitor is app
        // state, so defer both creation and collection until audio work requests
        // resource limits; never delay WebView startup for telemetry.
        info!("Deferring system monitor initialization until it is requested");
    ...
    pub async fn get_current_resources(&self) -> Result<SystemResources> {
        ...
        let system = system
            .as_ref()
            .expect("system monitor must be initialized by refresh_system_info");
