...
impl AudioStreamManager {
    ...
    /// Replace selected streams after their replacements have been opened.
    /// Starting first keeps the recording pipeline alive during a manual
    /// device change; callers stop the returned streams outside global locks.
    pub fn replace_streams(
        &mut self,
        microphone: Option<(AudioStream, Arc<AudioDevice>)>,
        system: Option<(AudioStream, Arc<AudioDevice>)>,
    ) -> (Option<AudioStream>, Option<AudioStream>) {
        let old_mic = microphone
            .map(|(stream, device)| {
                let old = self.microphone_stream.replace(stream);
        ...
        let old_system = system
            .map(|(stream, device)| {
                let old = self.system_stream.replace(stream);
